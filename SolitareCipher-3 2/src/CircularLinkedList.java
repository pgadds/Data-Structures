...
public class CircularLinkedList<E> implements Iterable<E> {
	...
	if (size == 0){ // head is added as the first item
		this.head = adding; // and tail is added as first item as well
		this.tail = adding; // add points to both the head and tail (look at drawing)
		this.tail.next = head;
	} else if (index == 0){
		tail.next = adding; // new head is added
		adding.next = head;
		head = adding;
	}
	else if (index == 0 ){
		tail.next = adding;  //new tail gets added
		adding.next = head;
		tail = adding;
	} else {
